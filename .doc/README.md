# dteam 文档导航

> **定位**：dteam 的文档体系，按 P0-P4 分层组织。

## 文档结构

```
.doc/
├── P0-原子层/        # 最基础的定义
├── P1-分子层/        # 原子组合
├── P2-细胞层/        # 分子组合
├── P3-组织层/        # 细胞组合
├── P4-用户接口层/    # 用户界面
├── 治理/             # 决策记录
├── 研究/             # 借鉴分析
└── 规范/             # 术语与文风
```

## P0-原子层

最基础的定义，不依赖其他概念。

| 文件 | 内容 | 说明 |
|------|------|------|
| [task.md](./P0-原子层/task.md) | task 数据模型 | 工作主语对象 |
| [signal.md](./P0-原子层/signal.md) | 信号定义 | 4信号+5策略=9种 |
| [memory.md](./P0-原子层/memory.md) | 内存定义 | 内存条目结构 |
| [status.md](./P0-原子层/status.md) | 状态定义 | 6个状态（pending/running/blocked/done/failed/cancelled） |
| [config.md](./P0-原子层/config.md) | 配置定义 | WorkerConfig |
| [i18n.md](./P0-原子层/i18n.md) | i18n 定义 | 国际化基础 |
| [loopDetector.md](./P0-原子层/loopDetector.md) | 循环检测器 | 防止渐进式参数变化 |
| [concurrency.md](./P0-原子层/concurrency.md) | 并发控制 | 最大并发数、超时控制 |
| [atomic.md](./P0-原子层/atomic.md) | 原子操作 | atomicCommit、atomicWrite |
| [gateChecks.md](./P0-原子层/gateChecks.md) | 门控检查 | 8项门控检查 + 四态判定 |
| [stateMachine.md](./P0-原子层/stateMachine.md) | 状态机 | 状态转换验证 |

## P1-分子层

原子组合，依赖 P0。

| 文件 | 内容 | 说明 |
|------|------|------|
| [角色.md](./P1-分子层/角色.md) | 角色定义 | 5个核心角色 |
| [explore.md](./P1-分子层/explore.md) | 探索者 | 搜集信息 |
| [design.md](./P1-分子层/design.md) | 方案制定者 | 制定方案 |
| [build.md](./P1-分子层/build.md) | 实现者 | 执行代码 |
| [check.md](./P1-分子层/check.md) | 验收者 | 检查质量 |
| [close.md](./P1-分子层/close.md) | 收口者 | 归档任务 |
| [工具接口.md](./P1-分子层/工具接口.md) | 工具接口 | task/reference/worker工具 |
| [信号总线.md](./P1-分子层/信号总线.md) | 信号总线 | 即时通信 |
| [共享内存.md](./P1-分子层/共享内存.md) | 共享内存 | 数据共享 |
| [TUI渲染层.md](./P1-分子层/TUI渲染层.md) | TUI渲染层 | 可视化 |
| [i18n翻译.md](./P1-分子层/i18n翻译.md) | i18n 翻译 | 多语言翻译 |
| [spawn.md](./P1-分子层/spawn.md) | spawn 子代理运行 | 子代理机制 |
| [共享内存工具.md](./P1-分子层/共享内存工具.md) | 共享内存工具 | 数据共享接口 |
| [信号工具.md](./P1-分子层/信号工具.md) | 信号工具 | 即时通信接口 |

## P2-细胞层

分子组合，依赖 P1。

| 文件 | 内容 | 说明 |
|------|------|------|
| [worker.md](./P2-细胞层/worker.md) | worker 概念 | 执行引擎 |
| [solo.md](./P2-细胞层/solo.md) | solo 模式 | 单任务 |
| [chain.md](./P2-细胞层/chain.md) | chain 模式 | 串行执行 |
| [team.md](./P2-细胞层/team.md) | team 模式 | 并行执行 |
| [信号总线.md](./P2-细胞层/信号总线.md) | 信号总线 | 即时通信 |

## P3-组织层

细胞组合，依赖 P2。

| 文件 | 内容 | 说明 |
|------|------|------|
| [worker编排层.md](./P3-组织层/worker编排层.md) | worker 编排层 | 编排架构 |
| [编排决策逻辑.md](./P3-组织层/编排决策逻辑.md) | 编排决策逻辑 | 选择模式 |

## P4-用户接口层

用户界面，依赖 P3。

| 文件 | 内容 | 说明 |
|------|------|------|
| [prompts.md](./P4-用户接口层/prompts.md) | prompts 定义 | 用户入口 |

## 治理

| 文件 | 内容 | 说明 |
|------|------|------|
| [决策记录.md](./治理/决策记录.md) | 决策记录 | D01-D11 |

## 研究

| 文件 | 内容 | 说明 |
|------|------|------|
| [GSD借鉴.md](./研究/GSD借鉴.md) | GSD 借鉴 | 设计理念 |
| [Superpowers借鉴.md](./研究/Superpowers借鉴.md) | Superpowers 借鉴 | 设计理念 |
| [dteam-vs-dflow-对比研究.md](./研究/dteam-vs-dflow-对比研究.md) | dteam vs dflow 对比研究 | 核心功能差异分析 |

## 规范

| 文件 | 内容 | 说明 |
|------|------|------|
| [术语与文风规范.md](./规范/术语与文风规范.md) | 术语与文风 | 写作规范 |
| [文件布局规范.md](./规范/文件布局规范.md) | 文件布局 | 目录结构 |

## 依赖关系

```
P0-原子层
    │
    ▼
P1-分子层
    │
    ▼
P2-细胞层
    │
    ▼
P3-组织层
    │
    ▼
P4-用户接口层
```

## 快速开始

1. 了解核心概念：[task.md](./P0-原子层/task.md) → [角色.md](./P1-分子层/角色.md) → [worker.md](./P2-细胞层/worker.md)
2. 了解执行模式：[solo.md](./P2-细胞层/solo.md) → [chain.md](./P2-细胞层/chain.md) → [team.md](./P2-细胞层/team.md)
3. 了解用户接口：[prompts.md](./P4-用户接口层/prompts.md)
