...
|---|---|---|---|---:|---|
| **T1 思考档** | 旗舰 | 高 | 全工具（读写改 + 审核） | 180 | 思考 / 决策 / 验收 / 回退重做 |
| **T2 标准档** | 标准 | 中 | 可写（读写改） | 120 | 常规实现 |
| **T3 快速档** | 快速 / 本地 | 低 | 默认只读；显式限定写 | 60 | 机械小任务 |

标准锚定**供应商产品线层级**（每家供应商都已分好旗舰/标准/快速三线）。dispatch 可覆盖默认配置（如 T3 临时给可写权限做独立模块小改）。worker 可通过 `dteam_signal` 申请一次额外的 60–120 次工作工具调用（10 的倍数），由主代理明确批准或拒绝；追加后仍耗尽时，主代理必须重新 dispatch fresh（全新）worker，不能再次追加。`dteam_signal` 本身不计入额度。
