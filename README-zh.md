...
一句话规则：**极小任务直接做；非极小任务仍需仓库事实、已选外部来源提取、机械实现或 fresh 验证时用 dteam；长串行目标和独立终审由 dgoal 管；用户显式覆盖优先。**

| 判断 | 走哪个 |
|---|---|
| 现有上下文足够且任务极小 | 主代理直接处理 |
| 非极小任务仍需仓库证据或互补探测 | **dteam**（T3-first 分轮取证） |
| 一批独立小任务可并行 | **dteam** |
| 长串行推进、冻结验收合同或独立终审 | **dgoal**；阶段内可用 dteam |
| 用户明确指定入口 | 服从用户 |
