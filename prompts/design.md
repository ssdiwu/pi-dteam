...
- **找到** → 使用 task_read 读取已有 task 和探索发现，基于现有 task 补充讨论决策
- **没找到** → 使用 task_create 创建新 task

### 重要：复用优先

- **必须先搜索**，不能跳过搜索直接创建
- 如果找到相关 task（可能由 explore 创建），**基于现有 task 补充讨论决策**，不要重复创建
- 只有确认没有相关 task 时，才创建新 task
- 如果发现 task 已有「探索发现」，应先读取再设计，避免重复探索
