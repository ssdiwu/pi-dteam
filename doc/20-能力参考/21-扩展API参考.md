...
>
> **当前状态注记**：`index.ts` 使用 `pi.registerTool` 注册五个模型工具 `dteam_dispatch`、`dteam_respond`、`dteam_control`、`dteam_recover`、`dteam_wait`，并注册 `/dteam` 管理命令。Worker Snapshot 的实时文本、thinking、当前工具和 timeout 诊断由 `/dteam` Modal 展示；不做跨 session 持久化或 resume。本文仍只记录 Pi 扩展 API 表面，不替代 `doc/10-架构与运行/12-API参考.md` 的 dteam 契约。
...
| 能力 | dteam 是否在用 | 备注 |
|------|:---:|------|
| `registerTool` | ✅ | `index.ts` 注册派发、普通回应、主动控制、超时恢复与显式等待五个工具 |
