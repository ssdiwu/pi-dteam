...
下一主版本删除 `dteam({ type: "dispatch" | "respond" })`，改为三个职责独立的模型工具：
...
`/dteam` 继续是用户查看、steering（插队指令）和确认取消的唯一入口；不新增 `dteam_status`、模型侧 cancel 或 steering。worker 的完成、失败和阻塞继续 follow-up（后续回传）给主模型，不引入轮询。`dteam_dispatch` 默认只显示紧凑受理摘要，按 `Ctrl+O` 才显示完整 JSON；模型上下文仍保留完整结构化结果。
