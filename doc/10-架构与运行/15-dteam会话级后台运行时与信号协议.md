...
- **列表默认页**：分为“运行中”和“历史记录”两个视图，默认运行中；用左右方向键或 `h/l` 切换。运行中视图只含 `queued` / `running` / `waiting`；历史视图只含终态 worker，并按终态时间倒序。每行优先显示 `title`、当前工具、档位轨迹、耗时和状态，避免把完整实时文本堆成列表卡片。
- **有界浏览**：列表与详情都限制可见行数，标题和操作提示钉住；用 `j/k`、方向键、`PgUp/PgDn`、`Home/End` 在超长内容中浏览，不依赖宿主 Modal 裁切后的外层滚动。
