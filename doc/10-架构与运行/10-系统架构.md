...
`dteam` 是 **Pi 里的多 subagent 协作引擎**——按 `goal` 自动决定 worker 数量与角色分工。

- **简单任务**：主 LLM 直接干（**不调 dteam**）
- **复杂任务**：主 LLM 调 `dteam(action="run", goal="...")`，planner 拆解任务后派 worker
- **运行方式：后台**——`dteam(action="run")` 立即返回 `runId`，**不阻塞主对话**；进度通过 `/dteam` 面板、widget、状态栏观察；完成后发 `dteam-report` 消息回到主对话
- **多 worker 协作**——planner 按 `solo` / `chain` / `team` 三种组织形式派 worker；5 个角色（`explore` / `design` / `build` / `check` / `close`），`build` 唯一能 `edit`（角色权限硬隔离）
- **后台执行不再依赖**已结束工具调用的 `onUpdate`（v0.4.1 起）

v0.4.1 收敛方向：把 dteam 明确成“后台任务系统”，不是“长时间占住当前工具调用的流式工具”。
