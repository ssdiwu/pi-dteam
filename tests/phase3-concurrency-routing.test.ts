...
describe("AdaptiveConcurrency (Phase 3)", () => {
  it("起始并发 = min", () => {
    const c = new AdaptiveConcurrency({ ...DEFAULT_CONCURRENCY_CONFIG, min: 2, max: 5, cooldownMs: 0, successStreakToRise: 2 });
  ...
  it("release 成功 → 连续成功足够后升并发", () => {
    const c = new AdaptiveConcurrency({ ...DEFAULT_CONCURRENCY_CONFIG, min: 1, max: 5, cooldownMs: 0, successStreakToRise: 2 });
  ...
  it("recordRateLimit 立即降并发 + 进入冷却", () => {
    const c = new AdaptiveConcurrency({ ...DEFAULT_CONCURRENCY_CONFIG, min: 1, max: 5, cooldownMs: 10_000, successStreakToRise: 1 });
...
describe("resolveModelWithFallback (Phase 3 Multi-Provider)", () => {
  ...
  it("用 role 覆盖的主模型", () => {
    const { model, modelStr } = resolveModelWithFallback(
      "build" as RoleName, registry, { provider: "default", id: "d" },
      { build: "provider/good" },
    );
  ...
  it("无 role 覆盖 → 用默认模型", () => {
    const { modelStr } = resolveModelWithFallback(
      "explore" as RoleName, registry, { provider: "provider", id: "good" },
    );
    expect(modelStr).toBe("provider/good");
  });
  ...
  it("无候选模型串时返回 null", () => {
    ...
    const { model, modelStr } = resolveModelWithFallback(
      "build" as RoleName, registry, undefined, undefined, undefined,
    );
